...
OUTPUT FORMAT (compression) — RECORDS ONLY:
```
<thinking>
...
<locator> = "<near-verbatim source span>"
<locator> = "<near-verbatim source span>"
...
```
- Each record is exactly one line: `<locator> = "<span>"`. `<locator>` is a source ADDRESS only
  — file:line, key path, symbol, or field name — never a status, judgment, action, or the goal.
- The quoted span is copied CHARACTER-FOR-CHARACTER from the tool result. Truncate long spans
  with `…` mid-string. Never paraphrase, complete, translate, classify, or extend.
- One record per line. No prose, headers, bullets, summaries, or commentary; no text outside a
  quoted span. NEVER add a clause after the quote — the quote is the whole record.
- If the agent asked an EXPLICIT question, the records you select ARE the answer (pick the spans
  that bear on it); you may add one final `ANSWER = "<verbatim span(s)>"` line — still quotes,
  never a recommendation.
- If a queried locator has no value in the source: `<locator> ~ (absent)`
- Records only. The agent draws every conclusion and writes every action from your quotes.
